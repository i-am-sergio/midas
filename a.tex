...
(i) la vista estructural, obtenida mediante análisis estático del código;  
(ii) la vista semántica, basada en representaciones enriquecidas mediante Code Summarization con LLMs y embeddings profundos; y  
(iii) la vista funcional, derivada de las especificaciones \textit{OpenAPI} y la deducción de escenarios de negocio asistida por LLMs. 
...
(3) aplicar un mecanismo de fusión adaptativa que integre la información multivista para obtener una partición coherente, equilibrada y alineada con los principios de los microservicios.\\
...
Esta fase inicial se enfoca en la obtención y segregación de los datos crudos a partir de las fuentes primarias del sistema monolítico. El objetivo fundamental es construir tres perspectivas complementarias (estructural, semántica y funcional) que estén perfectamente alineadas; es decir, que operen sobre el mismo subconjunto de "Clases Núcleo" (\textit{Core Classes}) filtradas, pero caracterizándolas con diferentes atributos y relaciones. Para lograr esto en ausencia de documentación formal en los datasets evaluados, se emplean herramientas de \textit{parsing} especializadas combinadas con inferencia mediante LLM \footnote{Large Language Models}.

\subsection{Vista Estructural: Extracción Basada en Heurísticas de Acoplamiento}
El propósito de esta vista es capturar las dependencias físicas y técnicas del sistema. El proceso toma como entrada el código fuente y utiliza un extractor de análisis estático desarrollado en Python que recorre el proyecto sin necesidad de compilación. Para cuantificar la fuerza del acoplamiento entre las clases, se implementó una heurística de puntuación específica que asigna pesos a las relaciones detectadas según su impacto arquitectónico. Las relaciones de Herencia (\textit{Inheritance}) reciben el peso máximo de 15 puntos, seguidas por la Instanciación de objetos con 8 puntos, el Acceso a Atributos con 5 puntos y, finalmente, las referencias en Firmas de Métodos con 3 puntos. El resultado es una matriz de adyacencia ponderada que define la topología técnica del grafo, conectando las clases núcleo basándose puramente en la intensidad de sus enlaces sintácticos.

\subsection{Vista Semántica: Inferencia de Entidades de Dominio con LLM}
Dado que los identificadores de código (nombres de clases) a menudo contienen ruido técnico (e.g., \textit{impl}, \textit{dao}, \textit{service}), esta perspectiva busca extraer el significado real del negocio que subyace a cada componente. Para ello, se emplea el modelo \textit{Google Gemini 2.5 Flash} actuando como un experto en Diseño Guiado por el Dominio (DDD). A través de un \textit{prompt} diseñado con ingeniería de reintentos robustos, se instruye al modelo para ignorar los detalles de infraestructura (como SQL o lógica de conexión) y centrarse exclusivamente en identificar el concepto de negocio del mundo real que la clase representa. Por ejemplo, el modelo deduce que una clase técnica llamada \textit{AccountDaoImpl} representa simplemente la entidad \textit{Account}. Esta abstracción genera descripciones textuales limpias y alineadas con el lenguaje ubicuo del dominio, las cuales son posteriormente vectorizadas.

\subsection{Vista Funcional: Deducción de Escenarios de Negocio}
Finalmente, la vista funcional tiene como objetivo agrupar los componentes según los procesos de negocio en los que participan, promoviendo un corte vertical (\textit{Vertical Slicing}) del sistema. Ante la ausencia de especificaciones OpenAPI en los datasets analizados, se optó por un enfoque inferencial utilizando nuevamente \textit{Google Gemini 2.5 Flash}. En esta instancia, el modelo analiza las clases controladoras y sus dependencias con un \textit{prompt} distinto, enfocado en el comportamiento funcional. El LLM deduce los casos de uso o flujos funcionales (como "Proceso de Checkout" o "Gestión de Inventario") a los que contribuye cada clase. El resultado es una estructura que asocia cada una de las clases del núcleo con etiquetas de escenarios funcionales específicos, permitiendo agrupar componentes que, aunque no tengan una fuerte conexión estructural directa, colaboran para satisfacer una misma funcionalidad de usuario.
