...
\begin{itemize}
    \item \textbf{RQ1:} ¿Cómo influye la fusión multivista auto-ponderada en la calidad estructural de la descomposición en microservicios?  

    \item \textbf{RQ2:} ¿Cómo afecta la fusión multivista al equilibrio y la complejidad de los microservicios resultantes?  

    \item \textbf{RQ3:} ¿Cuál es la contribución relativa de cada vista (estructural, semántica y funcional) dentro del proceso de fusión multivista?  
\end{itemize}
...
A continuación, se detalla el desempeño de MIDAS en comparación con los métodos baselines en los cuatro datasets evaluados.

La Tabla~\ref{tab:jpetstore_results} presenta los resultados para este sistema de comercio electrónico. MIDAS alcanza el ICP más bajo reportado (0.241), superando tanto a enfoques estructurales (GDC-DVF) como semánticos (GTMicro). Aunque el valor de Modularidad Estructural (SM = 0.171) es ligeramente inferior al máximo obtenido por GC-VCG, la reducción significativa en el acoplamiento sugiere que MIDAS ha logrado una separación más limpia de responsabilidades, evitando la trampa común de agrupar clases utilitarias con lógica de dominio para inflar artificialmente la cohesión.

%======================= JPETSTORE ===============================
\begin{table}[!ht] % H
\centering
\caption{Comparación de los diferentes métodos en el dataset JPetStore.}
\label{tab:jpetstore_results}
...
La Tabla~\ref{tab:daytrader_results} muestra los resultados para esta aplicación Java EE, caracterizada por su alta complejidad y uso intensivo de componentes EJB. En este escenario desafiante, MIDAS demuestra su robustez logrando simultáneamente la mayor modularidad estructural (0.195) y el menor acoplamiento (0.242). El hecho de superar a métodos basados en aprendizaje profundo (como DEEPLY y GC-VCG) indica que la integración explícita de la semántica es más efectiva para desenmarañar dependencias ocultas en arquitecturas densas que los enfoques puramente topológicos.

%========================= DAYTRADER ==============================
\begin{table}[!ht]
\centering
\caption{Comparación de los diferentes métodos en el dataset DayTrader.}
\label{tab:daytrader_results}
\begin{tabular}{l c c c c}
\toprule
\textbf{Método} & \textbf{SM} ($\uparrow$) & \textbf{ICP} ($\downarrow$) & \textbf{IFN} ($\downarrow$) & \textbf{NED} ($\downarrow$) \\
\midrule
...
Como se evidencia en la Tabla~\ref{tab:acmeair_results}, el desempeño en AcmeAir es prácticamente ideal. Con un ICP de 0.030 y un equilibrio perfecto de servicios (NED = 0.000), el algoritmo logró identificar con precisión los contextos delimitados naturales del dominio (Vuelos, Reservas, Autenticación). A diferencia de otros métodos que fragmentan excesivamente el sistema (aumentando IFN) o crean servicios gigantes, MIDAS mantuvo la integridad funcional de los módulos.

%========================= ACMEAIR ==============================
\begin{table}[!ht]
\centering
\caption{Comparación de los diferentes métodos en el dataset AcmeAir.}
\label{tab:acmeair_results}
\begin{tabular}{l c c c c}
\toprule
\textbf{Método} & \textbf{SM} ($\uparrow$) & \textbf{ICP} ($\downarrow$) & \textbf{IFN} ($\downarrow$) & \textbf{NED} ($\downarrow$) \\
\midrule
...
Finalmente, la Tabla~\ref{tab:plants_results} expone los resultados en Plants. Aquí, la propuesta maximiza tanto la modularidad (0.225) como la reducción de interfaces (0.833). El valor alto de NED (0.833) en este contexto no indica un fallo, sino una preferencia del algoritmo por una granularidad fina y altamente cohesiva, aislando servicios utilitarios pequeños en lugar de forzar una uniformidad de tamaño que violaría el principio de responsabilidad única.

%========================= PLANTS ==============================
\begin{table}[!ht]
\centering
\caption{Comparación de los diferentes métodos en el dataset Plants.}
\label{tab:plants_results}
\begin{tabular}{l c c c c}
\toprule
\textbf{Método} & \textbf{SM} ($\uparrow$) & \textbf{ICP} ($\downarrow$) & \textbf{IFN} ($\downarrow$) & \textbf{NED} ($\downarrow$) \\
\midrule
...
\label{subsec:estudio_ablacion}

Con el fin de determinar la contribución individual de cada fuente de información, se realizó un estudio de ablación comparando la versión completa de MIDAS (Fusión) frente a sus variantes de vista única: Estructural (MIDAS-Str), Semántica (MIDAS-Sem) y Funcional (MIDAS-Fun). Los resultados se resumen en la Tabla~\ref{tab:ablation}.

El análisis revela un patrón dual. Por un lado, las vistas aisladas ofrecen un rendimiento competente por sí mismas; esto valida la eficacia de la fase de preprocesamiento, donde el uso de LLMs (Gemini 2.5) limpia el ruido técnico del vocabulario, permitiendo que incluso una vista puramente semántica (MIDAS-Sem) logre agrupaciones coherentes.

Sin embargo, la superioridad de la fusión multivista se manifiesta en la gestión de la complejidad y los casos extremos. En \textbf{DayTrader}, donde la estructura es compleja y engañosa (ICP estructural de 0.399), la fusión integra la semántica para corregir dependencias espurias, reduciendo el acoplamiento a 0.242. En \textbf{AcmeAir}, donde las vistas aisladas tendían a soluciones desbalanceadas (NED alto), la fusión actuó como un mecanismo de consenso, estabilizando la arquitectura hacia un equilibrio perfecto (NED 0.000). En \textbf{Plants}, la fusión tuvo la inteligencia de no degradar el excelente rendimiento estructural, manteniendo los mejores valores. Esto confirma que el enfoque multivista no es simplemente aditivo, sino que aporta robustez y capacidad de corrección ante las deficiencias particulares de cada vista individual.
...
\subsection{Análisis del Mecanismo de Fusión}
\label{subsec:analisis_fusion}

Para comprender cómo el algoritmo prioriza la información, se extrajeron los pesos finales ($w_v$) asignados a cada vista durante la optimización. La Tabla~\ref{tab:midas_weights} resume estos valores y la Figura~\ref{fig:matrices_visual} ilustra visualmente el efecto de la integración en las matrices de similitud.
...
Los datos revelan que el mecanismo actúa como un filtro de calidad adaptativo. La vista estructural se establece sistemáticamente como la ``columna vertebral" del proceso ($w_{str} > 0.60$). Sin embargo, lo crucial es la varianza en los pesos complementarios. En sistemas con arquitectura explícita y limpia como \textbf{DayTrader} y \textbf{Plants}, el algoritmo margina casi totalmente las vistas semánticas (pesos < 0.06), confiando ciegamente en la estructura para evitar ruido. 

Por el contrario, en \textbf{AcmeAir} y \textbf{JPetStore}, donde la estructura técnica presenta mayor ambigüedad, la contribución combinada de las vistas semántica y funcional asciende hasta un 36\%. Esta intervención no es aleatoria; coincide precisamente con los casos donde la fusión logró mejoras drásticas en las métricas de acoplamiento y equilibrio (observadas en la sección anterior). Esto demuestra que el mecanismo de auto-ponderación posee la inteligencia para discernir cuándo la estructura es suficiente y cuándo requiere el contexto del negocio para refinar los límites de los microservicios.


% La Tabla~\ref{tab:midas_weights} revela un comportamiento consistente del algoritmo: la vista estructural es identificada sistemáticamente como la fuente de información más fiable para la partición, recibiendo el peso dominante ($w_{str} > 0.60$) en todos los casos. Sin embargo, la magnitud de esta dominancia varía según el sistema, lo que ofrece insights valiosos sobre la arquitectura de cada monolito.
...
A partir de los resultados expuestos, esta sección interpreta los hallazgos para responder a las preguntas de investigación planteadas.

\subsection{Sobre la Influencia en la Calidad Estructural (RQ1):}
Los resultados confirman que la fusión multivista auto-ponderada mejora significativamente la cohesión y, especialmente, el desacoplamiento de los microservicios. La métrica ICP (Porcentaje de Llamadas Internas) en MIDAS fue consistentemente inferior a la de los métodos del estado del arte en sistemas complejos como DayTrader y JPetStore. Esto se atribuye a la capacidad del algoritmo para integrar la semántica del dominio como un discriminador de dependencias. Mientras que los enfoques puramente estructurales se ven obligados a agrupar clases basándose en cualquier dependencia sintáctica fuerte (como clases de utilidad o DTOs compartidos), MIDAS utiliza la información semántica para identificar si esa dependencia representa una relación de negocio real. Al hacerlo, logra "romper" enlaces técnicos espurios, aislando componentes transversales y generando fronteras de servicio más limpias y mantenibles.

\subsection{Sobre el Equilibrio y la Complejidad (RQ2):}
Respecto al equilibrio de la arquitectura, la evidencia sugiere que la fusión multivista actúa como un mecanismo de regularización. En el caso de AcmeAir, donde las vistas estructurales y funcionales por separado proponían particiones desbalanceadas (NED alto), la fusión de ambas convergió en una solución de equilibrio ideal (NED = 0.000). Esto indica que la combinación de vistas mitiga los sesgos inherentes a una sola fuente de información: evita la creación de "God Classes" (común en análisis estructural puro) y la fragmentación excesiva (común en análisis semántico puro). Sin embargo, en sistemas como Plants, el modelo favoreció una granularidad fina (NED alto pero IFN bajo). Esto demuestra que el algoritmo no busca el equilibrio por sí mismo como un fin artificial, sino que adapta la complejidad de la solución a la estructura intrínseca del sistema, priorizando interfaces simples (IFN bajo) sobre tamaños uniformes.

\subsection{Sobre la Contribución de las Vistas y los Pesos (RQ3):}
El análisis del mecanismo de auto-ponderación revela que la vista estructural funciona como la columna vertebral del proceso, recibiendo sistemáticamente el mayor peso ($w_{str} > 0.60$). No obstante, la contribución de las vistas semántica y funcional varía dinámicamente según la claridad del sistema, tal como se muestra en la Tabla \ref{tab:midas_weights}. En sistemas donde la estructura técnica es ambigua o presenta alto acoplamiento (AcmeAir, JPetStore), el algoritmo asignó hasta un 36\% de peso combinado a las vistas semántica y funcional. Esta intervención fue determinante para corregir los límites de los servicios y reducir el acoplamiento. Por el contrario, en sistemas con una arquitectura técnica explícita (DayTrader, Plants), el modelo se replegó inteligentemente hacia la vista estructural ($w_{str} > 0.90$), minimizando el ruido semántico. Esto valida la hipótesis de que el mecanismo de fusión no es una simple suma, sino un filtro adaptativo que maximiza la relación señal-ruido.


\subsection{Comparativa con el Estado del Arte}
\label{subsec:resultados_rq1}

Se evaluó cómo se compara la calidad de la descomposición generada por MIDAS con la de los métodos \textit{baseline} descritos en la sección~\ref{subsec:baselines}.
