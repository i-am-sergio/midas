...
Como se describió en la Fase 4 de la propuesta, el número óptimo de microservicios ($k$) no se fija manualmente. Para determinarlo, se utiliza el \textbf{Coeficiente de Silhouette}. Esta métrica evalúa qué tan similar es un elemento (clase) a su propio clúster (cohesión) en comparación con otros clústeres (separación).
...
La Tabla~\ref{tab:rq2_ablation} presenta los resultados de esta comparación. Se resalta el mejor resultado para cada métrica en cada \textit{dataset}, permitiendo identificar en qué escenarios la integración de vistas supera a las fuentes de información aisladas.
...
En JPetStore ocurre algo similar, con un peso estructural de 0.738 y una contribución auxiliar del 26\%. Esto confirma que en sistemas donde la estructura técnica puede ser ambigua, el mecanismo auto-ponderado permite que la semántica y la funcionalidad actúen como factores de corrección ("tipping points") para decidir la pertenencia de clases fronterizas, sin sacrificar la solidez que ofrece el análisis estático.
