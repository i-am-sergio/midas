...
def run_spectral_clustering(fused_matrix_path, output_dir):
    ...
    # Convertir a array de numpy para sklearn
    affinity_matrix_U = df_fused.values

    # --- 2. Crear Matriz de Distancia (D) para Silhouette ---
    # El Coeficiente de Silhouette requiere una matriz de *distancia*.
    # Convertimos nuestra matriz de *afinidad/similitud* (U) en distancia (D).
    # D = 1 - U
    distance_matrix_D = 1 - affinity_matrix_U

    # --- 3. Iterar K y Evaluar ---
    
    # Rango de K: desde 2 hasta n-1 (como se solicitó)
    k_range = range(2, n_nodes)
    
    silhouette_scores = []
    all_cluster_assignments = [] # Lista para (k, class_name, cluster_id)

    print(f"Evaluando K desde 2 hasta {n_nodes - 1}...")

    for k in k_range:
        # a. Ejecutar Clustering Espectral
        # Usamos affinity='precomputed' porque le pasamos la matriz de afinidad U
        model = SpectralClustering(
            n_clusters=k,
            affinity='precomputed',
            assign_labels='kmeans', # Método estándar
            random_state=42         # Para reproducibilidad
        )
        
        # Entrenar sobre la matriz de AFINIDAD (U)
        model.fit(affinity_matrix_U)
        labels = model.labels_
        
        try:
            # b. Calcular Puntaje Silhouette
            # Usamos metric='precomputed' sobre la matriz de DISTANCIA (D)
        ...
        # Guardar asignaciones de clúster para este k, independientemente del cálculo de Silhouette
    ...
    if not df_scores.empty and df_scores['score'].notna().any():
        ...
        plt.legend()
        plt.grid(True)
        # Ajustar los ticks del eje X para que sean legibles
