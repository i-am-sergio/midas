...
private:
    ...
    void loadJavaFileContents() {
        for (auto& pair : classes_) {
            std::ifstream javaFile(pair.second.filename);
            if(javaFile.is_open()){
                std::stringstream buffer;
                buffer << javaFile.rdbuf();
                pair.second.fullContent = buffer.str();
            }
        }
    }
    ...
    void analyzeSignatures(const std::string& sourceClass, const ClassInfo& info, std::map<std::string, std::map<std::string, int>>& scores) {
        std::string combinedSigs = info.attributesStr + " " + info.methodsStr;
        std::regex typeRegex(R"(\b([A-Z][a-zA-Z0-9_]+)\b)");

        auto words_begin = std::sregex_iterator(combinedSigs.begin(), combinedSigs.end(), typeRegex);
